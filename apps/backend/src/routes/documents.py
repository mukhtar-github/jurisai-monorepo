...
async def list_documents(
    ...
    has_summary: Optional[bool] = Query(None, description="Filter by whether document has a summary"),
    has_entities: Optional[bool] = Query(None, description="Filter by whether document has extracted entities"),
    has_key_terms: Optional[bool] = Query(None, description="Filter by whether document has extracted key terms"),
    skip: int = Query(0, description="Number of records to skip for pagination"),
    limit: int = Query(100, description="Maximum number of records to return"),
    sort_by: str = Query("created_at", description="Field to sort by (created_at, updated_at, word_count, title)"),
    ...
    available_document_types = [dt[0] for dt in db.query(LegalDocument.document_type).distinct().all() if dt[0]]
    available_jurisdictions = [j[0] for j in db.query(LegalDocument.jurisdiction).distinct().all() if j[0]]
...
async def upload_document(
    ...
    auto_analyze: bool = Form(False, description="Automatically analyze the document after upload"),
    db: Session = Depends(get_db)
):
    """
        ...
        try:
            ...
            for term in key_terms:
                db_term = DocumentKeyTerm(
                    document_id=document.id,
                    term=term["term"],
                    relevance=float(term["relevance"]) * 100 if term["relevance"] is not None else None,
...
async def get_document(
    document_id: int,
    include_content: bool = Query(True, description="Whether to include the full document content"),
    include_metadata: bool = Query(False, description="Whether to include document metadata"),
    db: Session = Depends(get_db)
):
    """
...
async def analyze_document(
    ...
    try:
        if analysis_type == "entities":
            ...
            results = {
                "key_terms": key_terms,
                "stored_term_count": len(stored_terms)
            }
            ...
            results = {
                ...
                "key_terms": key_terms[:10],  # Return just first 10 terms in results for brevity
...
async def get_document_entities(
    ...
    sort_by: str = Query("relevance", description="Sort by field (relevance, text, type)"),
    sort_order: str = Query("desc", description="Sort order (asc, desc)"),
    db: Session = Depends(get_db)
):
    """
    ...
    # Format results
    entity_types = [et[0] for et in db.query(DocumentEntity.entity_type)
...
async def get_document_key_terms(
    document_id: int,
    min_relevance: Optional[float] = Query(None, description="Filter terms by minimum relevance score (0-100)"),
    min_frequency: Optional[int] = Query(None, description="Filter terms by minimum frequency"),
    limit: int = Query(50, description="Maximum number of terms to return"),
    offset: int = Query(0, description="Number of terms to skip"),
    sort_by: str = Query("relevance", description="Sort by field (relevance, term, frequency)"),
    sort_order: str = Query("desc", description="Sort order (asc, desc)"),
    db: Session = Depends(get_db)
):
    """
    ...
    ).filter(DocumentKeyTerm.document_id == document_id).first()
    ...
    return {
        ...
        "stats": {
            "avg_relevance": float(stats.avg_relevance) if stats and stats.avg_relevance else None,
            "max_relevance": float(stats.max_relevance) if stats and stats.max_relevance else None,
            "avg_frequency": float(stats.avg_frequency) if stats and stats.avg_frequency else None,
            "max_frequency": int(stats.max_frequency) if stats and stats.max_frequency else None
        }
    }
...
async def search_documents(
    ...
    search_strategy: str = Query("hybrid", description="Search strategy: 'semantic', 'lexical', or 'hybrid'"),
    ...
    use_semantic = search_strategy in ["semantic", "hybrid"] and RAG_AVAILABLE and rag_pipeline is not None
    ...
    if use_semantic:
        try:
            ...
            semantic_results = rag_pipeline.search(
                query=query,
                top_k=limit * 2,  # Get more results than needed to account for filtering
            ...
            semantic_scores = {int(result["id"]): result["score"] for result in semantic_results}
            
            # Query documents by IDs from semantic search
            semantic_docs = db.query(LegalDocument).filter(LegalDocument.id.in_(semantic_doc_ids)).all()
        ...
        if use_lexical:
            for result in lexical_results:
                ...
                if doc_id in combined_results:
                    ...
                    combined_results[doc_id]["score"] = combined_results[doc_id]["score"] * 1.2  # Boost score for hybrid matches
                else:
    ...
    for result in paginated_results:
        ...
        result["top_key_terms"] = [
            {"term": term.term, "relevance": term.relevance}
...
async def process_document_batch(files, document_type, jurisdiction, process_with_ai, auto_analyze, batch_id, db_url):
    """
            ...
            try:
                ...
                if process_with_ai:
                    try:
                        ...
                        logging.error(f"AI processing failed for {file.filename}: {str(e)}")
...
async def batch_upload_documents(
    ...
    background_tasks.add_task(
        ...
        batch_id=batch_id,
        db_url=str(db.bind.url),  # Pass database URL for creating new session in background task
    )
